...
if ($Module == 'manager_fio') {
    $user_id = $_SESSION['USER_ID'];
    $manager = mysqli_fetch_array(mysqli_query($CONNECT, "SELECT * FROM manager WHERE managerID='$user_id'"));
    $response = array(
        "fio"=>$manager['lastName'].' '.$manager['firstName'].' '.$manager['secondName']
    );
...
if ($Module == 'oauth_token' and $_POST['client_secret'] and $_POST['client_id'] and $_POST['response_type']) {
...
//        $refresh_token_old = $_POST['refresh_token'];
        $refresh_token_old = getallheaders();
        $refresh_token_old = $refresh_token_old['Authorization'];
        $refresh_token_old = substr($refresh_token_old, 7);

        $arr = mysqli_fetch_assoc(mysqli_query($CONNECT, "SELECT `client_id` FROM `oauth_apps`
    WHERE `client_secret`='$client_secret'"));
        $client_id = $arr['client_id'];
        if ($client_id == $client_id_post) {
            $arr = mysqli_fetch_assoc(mysqli_query($CONNECT, "SELECT `token`, `refresh_token`, `expired`, `code` FROM `oauth_apps_codes`
...
//                $refresh_token = GenPass($_POST['client_id'], $token);
                ...
                "error_description"=> "Application with this specified refresh_token not found or disabled."
            );
                header("HTTP/1.0 401 Unauthorized");
